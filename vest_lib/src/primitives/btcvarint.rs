...
impl<Output: OutputBuf, const MINIMAL: bool> Generator<Output, u64> for VarInt<MINIMAL> {
    ...
    fn generate_val(&mut self, g: &mut StdGen) -> u64 {
        ...
        match rand {
            0 => {
                g.rng.random_range(0..0xFD) as u64
            },
            1 => {
                g.rng.random_range(0xFD..=0xFFFF) as u64
            },
            2 => {
                g.rng.random_range(0x1_0000..=0xFFFF_FFFF as u32) as u64
            },
            _ => {
                g.rng.random_range(0x1_0000_0000 as u64..=0xFFFF_FFFF_FFFF_FFFF as u64) as u64
            },
        }
    }
}
